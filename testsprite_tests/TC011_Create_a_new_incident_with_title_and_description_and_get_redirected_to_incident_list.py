...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Iniciar Sesión' link to go to the login page (use element [95])
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/nav/div/div[3]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the test credentials into the login form: fill email (element [857]) with danidos@hotmail.com, fill password (element [864]) with DaNi26, then click the 'Iniciar sesión' button (element [865]).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Open the new incident form by clicking the 'Nueva Incidencia' button (interactive element [1256]).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/main/section/div/div/div/div[2]/a/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the incident title field, fill the incident description field, then submit the form.
        frame = context.pages[-1]
        # Input text
        ...
        await asyncio.sleep(3); await elem.fill('The hallway light on the 2nd floor has been flickering for two days and is now off.')
